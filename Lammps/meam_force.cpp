...
//              fprintf (pFile, "%e %e %e\n",rij, dUdrij,ddUddrij);
        ...
        if (!iszero(dscrfcn[fnoffset + jn])) {
          dUdsij = phi;// + frhop[i] * drho1ds1 + frhop[j] * drho1ds2; //--- Eq. 4.41(b)
           ddUddsij = 0.0;//frhopp[i] * drho1ds1 * drho1ds1 + frhop[i] * ddrho1dsds1 +
                      //frhopp[j] * drho1ds2 * drho1ds2 + frhop[j] * ddrho1dsds2;
//        if((i==0 and j==1) or (i==1 and j==0) )
//             fprintf(pFile, "%d %d %e %e %e\n",i,j,rho[i],frhop[i],frhopp[i]);
          
          
          for (m = 0; m < 3; m++) ddUdrijmds[m] = 0.0;//frhopp[i] * drho1ds1 * drho1drm1[m] + frhop[i] * ddrho1drmds1[m] +
        ...
        nv2 = 0;
        for (m = 0; m < 3; m++) {
          dUdrijm[m] = 0.0;//frhop[i] * drho1drm1[m] + frhop[j] * drho1drm2[m]; //--- Eq. 4.41(c)
          ddUdrdrijm[m] = 0.0;//frhopp[i] * drho1dr1 * drho1drm1[m] + frhop[i] * ddrho1drmdr1[m] + 
                           //frhopp[j] * drho1dr2 * drho1drm2[m] + frhop[i] * ddrho1drmdr2[m]; //--- deriv of Eq. 4.41(c) wrt r
          for (n = m; n < 3; n++) {
            ddUdrmdrn[nv2] =  0.0;//frhopp[i] * drho1drm1[m] * drho1drm1[n] + frhop[i] * ddrho1drmdrn1[nv2]+
