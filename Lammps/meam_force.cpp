...
//
        ...
        a1 = 2 * sij / rij;
        da1 = -a1/rij;
        drho1dr1 = 0.0;//2*rij;//a1 * (drhoa1j - rhoa1j / rij) * arg1i1; //--- 4.30(a)
        drho1dr2 = 0.0;//2*rij;//a1 * (drhoa1i - rhoa1i / rij) * arg1j1;
        ddrho1drdr1 = 0.0;//2.0;//da1 * (drhoa1j - rhoa1j / rij) * arg1i1 + 
...
//              fprintf (pFile, "%e %e %e %e %e\n",rij, sij, arg1i1, arg1i1, arg1i1_d);
       ddrho1drdr2 = 0.0;//2.0;//da1 * (drhoa1i - rhoa1i / rij) * arg1j1 + 
        ...
        da1 = -a1/rij;
        for (m = 0; m < 3; m++) {
          drho1drm1[m] = 2.0*delij[m];//0.0;//a1 * rhoa1j * arho1[i][m]; //--- 4.30(c)
          drho1drm2[m] = 2.0*delij[m];//0.0;//a1 * rhoa1i * arho1[j][m]; //--- d/drij[m]=-d/drji[m]
          ddrho1drmdr1[m] = 0.0;//da1 * rhoa1j * arho1[i][m] + a1 * drhoa1j * arho1[i][m] + a1 * rhoa1j * darho1dri[m];
          ddrho1drmdr2[m] = 0.0;//-da1 * rhoa1i * arho1[j][m] + a1 * drhoa1i * arho1[j][m] + a1 * rhoa1i * darho1drj[m];
        }
        // 
        ...
        for (m = 0; m < 3; m++) {
         for (n = m; n < 3; n++) {
          ddrho1drmdrn1[ nv2 ] = 2.0*(m==n?1:0);//0.0;//a1 * rhoa1j * darho1drni[nv2];
          ddrho1drmdrn2[ nv2 ] = 2.0*(m==n?1:0);//0.0;//a1 * rhoa1i * darho1drnj[nv2];
          nv2++;
         }
        }
...
//          fprintf (pFile, "%e %e %e %e\n",gamma[i],G_array[i],dG_array[i],ddG_array[i]);
        ...
        // rho1
          a1 = 2.0 / rij;
          da1 = -a1/rij;
          drho1ds1 = 0.0;//a1 * rhoa1j * arg1i1; //--- (4.30b)
          drho1ds2 = 0.0;//a1 * rhoa1i * arg1j1;
          ddrho1drds1 = 0.0;//da1 * rhoa1j * arg1i1 + a1 * drhoa1j * arg1i1 + a1 * rhoa1j * arg1i1_d;
          ddrho1drds2 = 0.0;//da1 * rhoa1i * arg1j1 + a1 * drhoa1i * arg1j1 + a1 * rhoa1i * arg1j1_d;
          ddrho1dsds1 = 0.0;//a1 * rhoa1j * darg1i1ds;
          ddrho1dsds2 = 0.0;//a1 * rhoa1i * darg1j1ds;
          a1 = 2.0 * sij / rij;
          da1 = 2.0/rij;
          for (m = 0; m < 3; m++) {
//            drho1drm1[m] = a1 * rhoa1j * arho1[i][m];
            ddrho1drmds1[m] = 0.0;//da1 * rhoa1j * arho1[i][m] + a1 * rhoa1j * darho1dsi[m];
            ddrho1drmds2[m] = 0.0;//-(da1 * rhoa1i * arho1[j][m]+ a1 * rhoa1i * darho1dsj[m]);
          }
        // rho2
          a2 = 2.0 / rij2;
...
//              fprintf (pFile, "%e %e %e\n",rij, dUdrij,ddUddrij);
        ...
        ddUdrijds = 0.0;
        if (!iszero(dscrfcn[fnoffset + jn])) {
          dUdsij = phi + frhop[i] * drho1ds1 + frhop[j] * drho1ds2; //--- Eq. 4.41(b)
           ddUddsij = frhopp[i] * drho1ds1 * drho1ds1 + frhop[i] * ddrho1dsds1 +
                      frhopp[j] * drho1ds2 * drho1ds2 + frhop[j] * ddrho1dsds2;
//        if((i==0 and j==1) or (i==1 and j==0) )
//             fprintf(pFile, "%d %d %e %e %e\n",i,j,rho[i],frhop[i],frhopp[i]);
          
          
          for (m = 0; m < 3; m++) ddUdrijmds[m] = frhopp[i] * drho1ds1 * drho1drm1[m] + frhop[i] * ddrho1drmds1[m] +
                                                  frhopp[j] * drho1ds2 * drho1drm2[m] + frhop[j] * ddrho1drmds2[m];
           ddUdrijds = phip + frhopp[i] * drho1ds1 * drho1dr1 + frhop[i] * ddrho1drds1 +
                              frhopp[j] * drho1ds2 * drho1dr2 + frhop[j] * ddrho1drds2;
        }
        nv2 = 0;
        for (m = 0; m < 3; m++) {
          dUdrijm[m] = frhop[i] * drho1drm1[m] + frhop[j] * drho1drm2[m]; //--- Eq. 4.41(c)
          ddUdrdrijm[m] = frhopp[i] * drho1dr1 * drho1drm1[m] + frhop[i] * ddrho1drmdr1[m] + 
                          frhopp[j] * drho1dr2 * drho1drm2[m] + frhop[i] * ddrho1drmdr2[m]; //--- deriv of Eq. 4.41(c) wrt r
          for (n = m; n < 3; n++) {
            ddUdrmdrn[nv2] =  frhopp[i] * drho1drm1[m] * drho1drm1[n] + frhop[i] * ddrho1drmdrn1[nv2]+
                              frhopp[j] * drho1drm2[m] * drho1drm2[n] + frhop[j] * ddrho1drmdrn2[nv2];
            nv2++;
          }  
         }
