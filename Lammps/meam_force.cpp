...
//
	...
	//arho3[i][nv3]=r[m].r[n].r[p].s
	//arho3b[i][m]=r[m].s
	rhoa3j=rhoa3i=1.0;drhoa3j=drhoa3i=0.0;
	nv3=0;
        for (n = 0; n < 3; n++) {
	  arho3b[i][n]=sij*delij[n]; //--- set arho3b
          for (p = n; p < 3; p++) {
            for (q = p; q < 3; q++) {
		    arho3[i][nv3]=sij*delij[n]*delij[p]*delij[q]; //---set arho3
              nv3++;
            }
	  }
	}	      
        ...
        A3j_d = 0.0;//drhoa3j / (rij2 * rij) - 3 * A3j / rij;
        A3i_d = 0.0;//drhoa3i / (rij2 * rij) - 3 * A3i / rij;
        nv3=0;
