...
//        dUdrij = phip;//(rij-1.0);
        force = dUdrij * recip + dUdsij * (-1.0/(rij*rij)) * recip;//dscrfcn[fnoffset + jn]; //-- recip = 1/r_{ij}
        for (m = 0; m < 3; m++) {
...
//         }
        ...
        if (vflag_atom != 0) {
          ...
          double ds = (-1.0/(rij*rij));//rij;//dscrfcn[fnoffset + jn] * rij; //???????
          double dds = 2.0/r3;//1.0;//ddscrfcn[fnoffset + jn];
