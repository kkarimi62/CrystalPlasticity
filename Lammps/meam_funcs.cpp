...
//-------------------
double
MEAM::Get_ddrho3drds( double rij, double sij, 
...
//-------------------
double
MEAM::Get_ddrho3dsds( double rij, double sij, 
                    double rhoa3j, double drhoa3j, double ddrhoa3j,
                    double arg1i3, double arg3i3,
                    double arg1i3_d, double arg3i3_d
                    ){
...
//-------------------------------
void
MEAM::Get_ddrho3drmds( int i,
                       double rij, double sij, double* delij,
                       double rhoa3j, 
                       double drhoa3j, 
                       double* darho3dri, double* darho3bdri,
                       double* ddrho3drmds1 //--- modify 
                     ){
   //rho3=arho3[i][nv3]*arho3[i][nv3]-(3/5)*arho3b[i][n]*arho3b[i][n]
   double a3 = 6.0 * sij / rij3;
   double da3 = 6.0 / rij3;
   double a3a = 6.0 * sij / (5.0 * rij);
   double da3a = 6.0 / (5.0 * rij);
   int m, n, p, nv2;
   double xsumi;
   for (m = 0; m < 3; m++) {
      ddrho3drmds1[m] = 0.0;
   ...
   ddrho3drmds1[m] = (a3 * ddrho3drmds1[m] - a3a * darho3bdsi[m]) * rhoa3j;
          //drho3drm1[m] = (a3 * drho3drm1[m] - a3a * arho3b[i][m]) * rhoa3j;
   }
}
//-------------------------------
...
//-------------------------------
void
MEAM::Get_ddrho3drij_mdrik_n( int i,
                       double rij, double sij, double* delij,
                       double rhoa3j, 
                       double* ddrho3drmdrn1 //--- modify 
                     ){
...
//     }
   ...
   double dsg_alpha_beta_dr = ((-recip2*(dUdrij+dUdsij*ds)+recip*(ddUddrij+ddUdrijds*ds+dUdsij*dds))*delij[alpha]+ddUdrdrijm[alpha])*delij[beta];
   double dsg_alpha_beta_ds = (recip*(ddUdrijds+ddUddsij*ds)*delij[alpha]+ddUdrijmds[alpha])*delij[beta];
//    if(i==0 and j==1) printf("i=%d,j=%d,dsg_alpha_beta_ds=%e\t%e\t%e\t%e\n",i,j,dsg_alpha_beta_ds,ddUdrijds,ddUddsij,ddUdrijmds[alpha]);
...
//   }
      ...
      if(i==0 and j==1 and alpha==0 and beta==1){
         ...
         fprintf( pFile,"#rik\trjk\trij\tsij\tdelij[0]\tdelij[1]\tdelij[2]\tdsgdrm[0]\tdsgdrm[1]\tdsgdrm[2]\tdsg_dr\tdsg_ds\tdsg_drik\tdsg_drjk\n");
         fprintf( pFile,"%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\n",rik,rjk,rij,sij,delij[0],delij[1],delij[2],dsg_alpha_beta_drm[0],dsg_alpha_beta_drm[1],dsg_alpha_beta_drm[2],dsg_alpha_beta_dr,dsg_alpha_beta_ds,dsg_alpha_beta_drik*rik,dsg_alpha_beta_drjk*rjk);
         fclose(pFile);
      }
