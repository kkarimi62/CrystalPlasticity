                ...
                int* firstneigh, int numneigh_full, int* firstneigh_full, int /*ntype*/, int* type, int* fmap)
{
  ...
  for (jn = 0; jn < numneigh; jn++) {
    ...
    if (!iszero(sfcij) && !isone(sfcij)) {
      ...
      ddscrfcn[jn] = dscrfcn[jn]+rij*ddscrfcn[jn];//--- units of s/r^2
...
//          arho3b[j][m] = arho3b[j][m] -  delij[m];
 ...
 //             arho3[i][nv3] = arho3[i][nv3] + 1 * delij[m] * delij[n] * delij[p];
              arho3[j][nv3] = arho3[j][nv3] - A3i * delij[m] * delij[n] * delij[p];
 //             arho3[j][nv3] = arho3[j][nv3] - 1 * delij[m] * delij[n] * delij[p];
              nv3 = nv3 + 1;
            }
          }
        }
