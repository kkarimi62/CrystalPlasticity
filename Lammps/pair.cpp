...
void Pair::ev_tally(int i, int j, int nlocal, int newton_pair,
                   ...
                   double stiff, double n0, double n1,double n2, double rdudr)
{
  ...
  if (vflag_either) {
    ...
    v[5] = dely*delz*fpair;
    //--- per-atom modulus
    vm[ 0 ]  = -0.5 * (stiff * n0 * n0 * n0 * n0+rdudr*n0 * n0);
    ...
    vm[ 10 ] = -0.5 * (stiff * n1 * n1 * n1 * n2+rdudr*n1 * n2);
    //
    vm[ 11 ] = -0.5 * (stiff * n2 * n2 * n2 * n2+rdudr*n2 * n2);
    vm[ 12 ] = -0.5 * stiff * n2 * n2 * n0 * n1;
    vm[ 13 ] = -0.5 * stiff * n2 * n2 * n0 * n2;
    vm[ 14 ] = -0.5 * stiff * n2 * n2 * n1 * n2;
    //
    vm[ 15 ] = -0.5 * (stiff * n0 * n1 * n0 * n1+rdudr*n1 * n1);
    vm[ 16 ] = -0.5 * (stiff * n0 * n1 * n0 * n2+rdudr*n1 * n2);
    vm[ 17 ] = -0.5 * stiff * n0 * n1 * n1 * n2;
    //
    vm[ 18 ] = -0.5 * (stiff * n0 * n2 * n0 * n2+rdudr*n2 * n2);
    vm[ 19 ] = -0.5 * stiff * n0 * n2 * n1 * n2;
    //
    vm[ 20 ] = -0.5 * (stiff * n1 * n2 * n1 * n2+rdudr*n2 * n2);  
    if (vflag_global) {
      if (newton_pair) {
        virial[0] += v[0];
        virial[1] += v[1];
...
double Pair::memory_usage()
{
  ...
  bytes += comm->nthreads*maxvatom*(6+21) * sizeof(double);
